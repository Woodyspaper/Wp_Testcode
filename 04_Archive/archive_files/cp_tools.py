...
SELECT {top}
    ...
    CUST_TYP, CATEG_COD, PROF_COD_1, TERMS_COD, TAX_COD
FROM dbo.AR_CUST
...
def customer_to_woo_payload(c):
    ...
    return {
        ...
        "meta_data": [
            ...
            {"key": "cp_prof_cod_1", "value": c.get("PROF_COD_1", "")},
        ],
    }
...
def main():
    ...
    if cmd == "customers":
        ...
        print(f"\n{'CUST_NO':<15} {'NAME':<30} {'EMAIL':<30} {'CATEG':<10} {'TIER':<10}")
        print("-" * 100)
        for c in customers:
            print(f"{c['CUST_NO']:<15} {(c.get('NAM') or '')[:30]:<30} "
                  f"{(c.get('EMAIL_ADRS_1') or '')[:30]:<30} {c.get('CATEG_COD') or '':<10} {c.get('PROF_COD_1') or '':<10}")
