...
# Monitors inventory sync logs and task status
...
Write-Host "Inventory Sync Monitor" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    $task = Get-ScheduledTask -TaskName "WooCommerce Inventory Sync" -ErrorAction SilentlyContinue
    if ($task) {
        $taskInfo = Get-ScheduledTaskInfo -TaskName "WooCommerce Inventory Sync"
...
if (Test-Path $logDir) {
    ...
    $logFiles = Get-ChildItem -Path $logDir -Filter "woo_inventory_sync_*.log" | 
...
$recentLogs = Get-ChildItem -Path $logDir -Filter "woo_inventory_sync_*.log" -ErrorAction SilentlyContinue | 
...
Write-Host "   Example: .\Monitor-InventorySync.ps1 -ShowDetails" -ForegroundColor Gray
