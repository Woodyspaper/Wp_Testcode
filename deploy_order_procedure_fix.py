"""Deploy the fixed sp_CreateOrderFromStaging procedure with shipping address support"""
from database import get_connection
import re

# Read the fixed procedure
with open('01_Production/sp_CreateOrderFromStaging.sql', 'r', encoding='utf-8') as f:
    sql = f.read()
...
drop_sql = "IF OBJECT_ID('dbo.sp_CreateOrderFromStaging', 'P') IS NOT NULL DROP PROCEDURE dbo.sp_CreateOrderFromStaging;"
...
try:
    ...
    conn.commit()
    print("[OK] Procedure created successfully with shipping address support")
    
    # Verify
    cursor.execute("SELECT name FROM sys.procedures WHERE name = 'sp_CreateOrderFromStaging'")
